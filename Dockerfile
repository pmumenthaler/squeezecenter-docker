...
RUN curl -Lsf -o /tmp/logitechmediaserver.noarch.rpm http://downloads.slimdevices.com/LogitechMediaServer_v7.9.2/logitechmediaserver-7.9.2-1.noarch.rpm
RUN useradd -u 1032 squeezeboxserver
RUN yum install -y /tmp/logitechmediaserver.noarch.rpm && rm -f /tmp/logitechmediaserver.noarch.rpm && yum install -y perl-IO-Socket-SSL.noarch perl-IO-Zlib.noarch
...
ENV LANG=en_US.UTF-8 \
    ...
    SQUEEZEBOX_CACHE_DIR="/var/cache" \ 
